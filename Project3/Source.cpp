...
void removeCrewMember(vector<Expedition>& expeditions) {
    cout << "Enter the ID of the expedition to remove a crew member from: ";
    int id;
    cin >> id;
    ...
    auto it = remove_if(expedition->crewMembers.begin(), expedition->crewMembers.end(), [&name](const CrewMember& crewMember) {
        return crewMember.name == name;
        });

    if (it != expedition->crewMembers.end()) {
        expedition->crewMembers.erase(it, expedition->crewMembers.end());
